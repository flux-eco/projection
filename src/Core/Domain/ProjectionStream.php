...
class ProjectionStream
{
    ...
    private ?int $sequenceOffSet = null;
    ...
    final public function loadProjectedRows(?int $sequenceOffSet = null, ?int $limit = null) : void
    {
        $queriedRows = $this->outbounds->queryProjectionStorage($this->projectionName, $this->schema, null,
            $sequenceOffSet, $limit);
    ...
    final public function projectData(string $projectionId, array $data) : void
    {
        ...
        $schema = $this->outbounds->getProjectionSchema($this->projectionName);
        $fulltextSearchKeys = [];
        if (key_exists('fulltextSearch', $schema) === true) {
            $fulltextSearchKeys = $schema['fulltextSearch'];
        }

        $fulltextValues = [];
        foreach ($data as $key => $value) {
            if (in_array($key, $fulltextSearchKeys) === true) {
                $fulltextValues[] = $value;
            }
            $this->applyAndRecord($projectionId, $key, $value);
        }

        if (count($fulltextValues) > 0) {
            $this->applyAndRecord($projectionId, 'fulltextSearch', implode('  ', $fulltextValues));
        }
